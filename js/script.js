...
const portfolioItems = [
  ...
  {cat:'Travel', name:'Nusantara Trip', color:'var(--secondary)'},
  {cat:'Villa', name:'Villa Kayu Manis', color:'var(--accent)'},
  {cat:'Homestay', name:'Homestay Alam Sari', color:'var(--magenta)'},
  {cat:'Restaurant', name:'Rasa Nusantara', color:'var(--primary)'},
  {cat:'Cafe', name:'Kopi Senja', color:'var(--secondary)'},
  {cat:'Rental Motor', name:'Rental Jaya Motor', color:'var(--accent)'},
  {cat:'Rental Mobil', name:'Prime Car Rental', color:'var(--magenta)'},
  {cat:'Website Pribadi', name:'Portfolio Andra', color:'var(--primary)'},
  {cat:'Undangan Digital', name:'Nadia & Raka', color:'var(--secondary)'},
];
...
function renderPortfolio(filter){
  ...
  portfolioItems.filter(p => filter==='Semua' || p.cat===filter).forEach((p,i) => {
    ...
    <div class="reveal-scale group card rounded-2xl overflow-hidden cursor-pointer" style="transition-delay:${(i%3)*0.08}s">
      <div class="h-44 relative overflow-hidden" style="background:linear-gradient(135deg,${p.color},var(--secondary))">
        <div class="absolute inset-0 flex items-center justify-center opacity-90 group-hover:scale-110 transition-transform duration-500">
          <i data-lucide="layout-template" class="w-14 h-14 text-white/70"></i>
        </div>
